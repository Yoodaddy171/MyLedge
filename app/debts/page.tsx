...
export default function DebtsPage() {
  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    // VALIDATION: Prevent payment exceeding remaining debt
    ...
    if (error) toast.error(error.message);
      ...
      fetchDebts(); // Refresh to see updated remaining amount
    }
  };
    ...
    <div className="max-w-6xl mx-auto pb-20 text-black font-black uppercase">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <button onClick={() => { 
          ...
          setIsModalOpen(true); 
        }} className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 bg-red-600 rounded-xl text-[10px] font-black text-white hover:bg-red-700 shadow-xl uppercase tracking-widest transition-all">
              ...
              >
                ...
                <div className="flex gap-2 sm:opacity-0 group-hover:opacity-100 transition-all border-l border-slate-50 pl-6 h-10 items-center">
                  {debt.remaining_amount > 0 && (
                    <button onClick={() => openPayModal(debt)} className="px-4 py-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 font-black text-[10px] uppercase tracking-widest rounded-lg transition-colors border border-emerald-200">
                      Pay
                    </button>
                  )}
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 overflow-hidden text-black font-black">
              ...
              <form onSubmit={handlePaySubmit} className="space-y-6">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Paying For</label>
                  <div className="p-4 bg-slate-50 rounded-xl border-2 border-slate-100">
                    <p className="font-black text-sm">{selectedDebtForPay?.name}</p>
                    <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase">Rem: {new Intl.NumberFormat('id-ID').format(selectedDebtForPay?.remaining_amount)}</p>
                  </div>
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Payment Amount</label>
                  <input type="text" placeholder="0" required className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-black shadow-sm outline-none focus:border-emerald-500 transition-all" value={formatDisplayAmount(payForm.amount)} onChange={(e) => setPayForm({ ...payForm, amount: e.target.value.replace(/\D/g, "") })} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Pay From (Wallet)</label>
                  <select required className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-xs font-black uppercase shadow-sm outline-none focus:border-emerald-500" value={payForm.wallet_id} onChange={(e) => setPayForm({ ...payForm, wallet_id: e.target.value })}>
                ...
                <div>
                  ...
                  <input type="date" required className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-black shadow-sm outline-none focus:border-emerald-500 transition-all" value={payForm.date} onChange={(e) => setPayForm({ ...payForm, date: e.target.value })} />
                </div>
                <button type="submit" className="w-full bg-emerald-600 text-white font-black py-4 rounded-xl shadow-xl hover:bg-emerald-700 transition-all mt-4 uppercase tracking-widest text-[10px]">Confirm Payment</button>
              </form>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 overflow-hidden text-black font-black max-h-[95vh] overflow-y-auto overscroll-contain" style={{WebkitOverflowScrolling: 'touch'}}>
              <div className="flex justify-between items-center mb-8">
              ...
              <form onSubmit={handleManualSubmit} className="space-y-6">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Debt Name</label>
                  <input type="text" placeholder="e.g. Car Loan, Credit Card" required className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-black shadow-sm outline-none focus:border-red-500 transition-all" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Creditor (Bank/Person)</label>
                  <input type="text" placeholder="e.g. BCA, John Doe" className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-black shadow-sm outline-none focus:border-red-500 transition-all" value={formData.creditor} onChange={(e) => setFormData({ ...formData, creditor: e.target.value })} />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Monthly Payment</label>
                    <input type="text" placeholder="0" className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-black shadow-sm outline-none focus:border-red-500 transition-all" value={formatDisplayAmount(formData.monthly_payment)} onChange={(e) => setFormData({ ...formData, monthly_payment: e.target.value.replace(/\D/g, "") })} />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Interest Rate (%)</label>
                    <input type="text" placeholder="0" className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-black shadow-sm outline-none focus:border-red-500 transition-all" value={formData.interest_rate} onChange={(e) => setFormData({ ...formData, interest_rate: e.target.value })} />
                  </div>
                </div>
                <div>
