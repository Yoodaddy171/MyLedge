...
export default function InvestmentsPage() {
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { user_id: user.id, type: formData.type.toLowerCase(), symbol: formData.symbol.toUpperCase().trim(), quantity: Number(formData.quantity), avg_buy_price: Number(formData.avg_buy_price), current_price: marketPrice, last_price_update: new Date().toISOString() };
      if (editingId) await supabase.from('assets').update(payload).eq('id', editingId);
      else await supabase.from('assets').upsert(payload, { onConflict: 'user_id,symbol,type' });
  ...
  async function syncMarketPrices() {
    try {
      ...
      const result = await response.json();
      for (const update of result.updatedAssets) { await supabase.from('assets').update({ current_price: update.current_price, last_price_update: new Date().toISOString() }).eq('id', update.id); }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return;
        const payload = data.map(row => ({ user_id: user.id, type: String(row['Type']).trim().toLowerCase(), symbol: String(row['Symbol']).trim().toUpperCase(), quantity: Number(row['Quantity'] || 0), avg_buy_price: Number(row['Avg Buy Price'] || 0), last_price_update: new Date().toISOString() })).filter(a => a.symbol && a.quantity > 0);
        await supabase.from('assets').upsert(payload, { onConflict: 'user_id,symbol,type' });
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-sm rounded-2xl shadow-2xl p-8 overflow-hidden text-black font-black max-h-[95vh] overflow-y-auto overscroll-contain" style={{WebkitOverflowScrolling: 'touch'}}>
