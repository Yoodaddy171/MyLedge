...
export default function TasksPage() {
  ...
  const [formData, setFormData] = useState({ title: '', category: 'Kegiatan Pribadi', priority: 'Sedang', status: 'todo', deadline: '', notes: '' });
  ...
  const openEditModal = (task: any) => {
    ...
    setFormData({ 
      title: task.title, 
      category: task.category || 'Kegiatan Pribadi', 
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    if (error) toast.error(error.message);
      ...
      setFormData({ title: '', category: 'Kegiatan Pribadi', priority: 'Sedang', status: 'todo', deadline: '', notes: '' });
      fetchTasks();
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 text-black font-black uppercase">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
        ...
        <button onClick={() => { setEditingId(null); setFormData({ title: '', category: taskCategories[0]?.name || 'Kegiatan Pribadi', priority: 'Sedang', status: 'todo', deadline: '', notes: '' }); setIsModalOpen(true); }} className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 rounded-xl text-[10px] font-black text-white hover:bg-blue-700 shadow-xl transition-all uppercase tracking-widest">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-10">
        <MiniMetric title="Pending" value={stats.todo} icon={<Clock size={16} />} color="text-slate-600" />
        <MiniMetric title="Progress" value={stats.inProgress} icon={<Activity size={16} />} color="text-blue-600" />
        ...
        <MiniMetric title="Total" value={stats.total} icon={<Target size={16} />} color="text-slate-900" />
      </div>
      ...
      <div className="flex flex-col gap-6 mb-10">
        <div className="flex items-center justify-between pt-4 pb-4 border-b-2 border-slate-50 overflow-x-auto scrollbar-hide px-2">
          ...
          <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-100 ml-8">
            ...
            <FilterButtonLite active={filter === 'in_progress'} onClick={() => setFilter('in_progress')} label="Progress" />
                ...
                <div className="flex items-center">
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl border-2 flex items-center justify-center transition-all ${
                      ...
                      'bg-slate-50 border-slate-100 text-transparent hover:border-blue-500 hover:bg-white'
                    }`}
                  >
                    {task.status === 'done' ? <CheckCircle2 size={20} /> : 
                     task.status === 'in_progress' ? <Activity size={20} /> : 
                     <Clock size={20} className="text-slate-200 group-hover:text-blue-500" />}
                  </button>
                </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-10 overflow-hidden text-black font-black max-h-[95vh] overflow-y-auto overscroll-contain" style={{WebkitOverflowScrolling: 'touch'}}>
              ...
              <form onSubmit={handleManualSubmit} className="space-y-8">
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-3 block">Current Status</label>
                  <div className="flex p-1 bg-slate-50 rounded-[1.2rem] border-2 border-slate-100">
                    {['todo', 'in_progress', 'done'].map((s) => (
                      <button
                        ...
                        className={`flex-1 py-3 rounded-[1rem] text-[9px] font-black uppercase tracking-widest transition-all ${
                          formData.status === s
                            ? s === 'done' ? 'bg-emerald-500 text-white shadow-lg'
                            : s === 'in_progress' ? 'bg-blue-500 text-white shadow-lg'
