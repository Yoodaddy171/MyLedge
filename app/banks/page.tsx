...
export default function BanksPage() {
  ...
  async function fetchBanks() {
    try {
      setLoading(true);
      // V2 Migration: Use wallet_balances_view for auto-calculated values
      ...
      // Map view data to component state
      // View returns: current_balance, total_income, total_expense
      const enrichedBanks = data?.map(w => ({
        ...w,
        balance: w.current_balance,
        income: w.total_income,
        expense: w.total_expense,
        // View doesn't have trxCount currently, but that's minor.
        // We can either add it to view or ignore/fetch separately.
        // For now, let's leave trxCount as 0 or remove it from UI if not critical.
        trxCount: 0 
      }));
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      current_balance: parseFloat(formData.initial_balance) || 0 // Set initial current balance same as initial
    };

    try {
      if (editingId) {
        // Don't update current_balance on edit, let triggers/logic handle it or only update name/type
        // But for V2 simple edit, we might want to update initial_balance which re-triggers calc?
        // For now exclude current_balance from update to avoid overwriting calculated values
                        ...
                        <button
                          onClick={() => { 
                            ...
                            setFormData({ 
                              name: item.name,
                              type: item.type || 'bank',
                              initial_balance: (item.initial_balance || 0).toString(),
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-md rounded-[3rem] shadow-2xl max-h-[90vh] overflow-hidden text-black font-black">
              <div className="flex justify-between items-center px-10 py-8 border-b-2 border-slate-100">
              ...
              <div className="overflow-y-auto overscroll-contain max-h-[calc(90vh-120px)] p-10">
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Account Name</label>
                  <input
                    type="text"
                    placeholder="e.g. BCA, Wallet, Cash"
                    required
                    className="w-full bg-white border-2 border-slate-100 rounded-[1.2rem] px-6 py-4 text-sm font-bold text-black focus:border-blue-500 outline-none transition-all shadow-sm"
                    value={formData.name}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      required
                      className="w-full bg-white border-2 border-slate-100 rounded-[1.2rem] px-6 py-4 text-sm font-bold text-black focus:border-blue-500 outline-none transition-all shadow-sm appearance-none"
                      value={formData.type}
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Currency</label>
                    <select
                      required
                      className="w-full bg-white border-2 border-slate-100 rounded-[1.2rem] px-6 py-4 text-sm font-bold text-black focus:border-blue-500 outline-none transition-all shadow-sm appearance-none"
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-2 block">Initial Balance</label>
                  <input
                    type="number"
                    placeholder="0"
                    className="w-full bg-white border-2 border-slate-100 rounded-[1.2rem] px-6 py-4 text-sm font-bold text-black focus:border-blue-500 outline-none transition-all shadow-sm"
                ...
                <button type="submit" className="w-full bg-black text-white font-black py-5 rounded-[1.5rem] shadow-xl hover:bg-slate-800 transition-all mt-4 uppercase tracking-widest text-[10px]">Verify & Save</button>
              </form>
              </div>
            </motion.div>
          </div>
        ...
        {selectedBankForActivity && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }} className="relative bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl overflow-hidden text-black font-black h-[90vh] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto overscroll-contain p-10 pt-6 space-y-4" style={{WebkitOverflowScrolling: 'touch'}}>
                {loadingActivity ? (
